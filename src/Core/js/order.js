...
jQuery(document).on('ajaxComplete', function (event, xhr, settings) {
	...
	function check_selection() {
		...
		jQuery.ajax({
			...
			if (response && (response.type === 'unavailable' || response.type === 'no_orders' || response.type === 'no_permission')) {
	...
	function show_pointer(e) {
		...
		const pixelimgPos = jQuery('#pixelimg').offset();
		let OffsetX = e.pageX - pixelimgPos.left;
		let OffsetY = e.pageY - pixelimgPos.top;

		OffsetX -= window.pointer_width / 2;
		OffsetY -= window.pointer_height / 2;
		...
		if (OffsetY + window.pointer_height > grid_height) {
			OffsetY = grid_height - window.pointer_height;
		}

		if (OffsetX + window.pointer_width > grid_width) {
			OffsetX = grid_width - window.pointer_width;
		}

		window.$block_pointer.css('top', OffsetY + 'px');
		window.$block_pointer.css('left', OffsetX + 'px');
