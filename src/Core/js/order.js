...
jQuery(document).on('ajaxComplete', function (event, xhr, settings) {
	...
	function check_selection() {
		...
		const gridWidthBlocks = parseInt(MDS_OBJECT.grid_width || gridData.grid_width_blocks);
		const currentImgWidth = window.$pixelimg.width();
		const originalImgWidth = parseInt(gridData.orig_width_px || (gridWidthBlocks * gridData.block_width));
		...
		jQuery.ajax({
			...
			if (response) {
				// Error responses
				if (response.type === 'unavailable' || response.type === 'no_orders' || response.type === 'no_permission') {
	...
	function make_selection(event) {
		...
		jQuery.ajax({
			...
			if (response) {
				if (response.type === 'unavailable' || response.type === 'max_orders') {
					alert(response.data.value);
...
function scaleImageMap() {
	...
	const gridData = MDS_OBJECT.grid_data || {};
	const gridWidthBlocks = parseInt(gridData.grid_width_blocks || MDS_OBJECT.grid_width);
	const gridHeightBlocks = parseInt(gridData.grid_height_blocks || MDS_OBJECT.grid_height);
	...
	if (window.$block_pointer) {
		...
		const origWidth = parseInt(window.$block_pointer.attr('data-original-width')) || gridData.pointer_width || blockWidth * 4;
		const origHeight = parseInt(window.$block_pointer.attr('data-original-height')) || gridData.pointer_height || blockHeight * 4;
		...
		const origGridWidth = gridData.orig_width_px || (gridWidthBlocks * gridData.block_width);
		...
		if (window.$block_pointer.data('block_x') !== undefined) {
			...
			if (gridData.debug_mode) {
				console.log('ScaleImageMap:', {
					grid: { blocks: gridWidthBlocks + 'x' + gridHeightBlocks, 
						pixels: currentWidth + 'x' + currentHeight },
					block: { width: blockWidth, height: blockHeight },
					pointer: { original: origWidth + 'x' + origHeight,
						actual: pointerWidth + 'x' + pointerHeight },
					position: { pixel: pixelX + ',' + pixelY, 
...
function get_clicked_block() {
	...
	const gridData = MDS_OBJECT.grid_data || {};
	const gridWidthBlocks = parseInt(MDS_OBJECT.grid_width || gridData.grid_width_blocks);
	const gridHeightBlocks = parseInt(MDS_OBJECT.grid_height || gridData.grid_height_blocks);
	...
	if (window.$block_pointer) {
		...
		if (blockX < 0 || blockY < 0 || blockX >= gridWidthBlocks || blockY >= gridHeightBlocks) {
		...
		if (gridData.debug_mode) {
			console.log('Selection:', {
				pointer: { left: pointerOffset.left, top: pointerOffset.top, width: pointerWidth, height: pointerHeight },
	...
	// Rebinding events
	jQuery(document).off('click', '#pixelimg').on('click', '#pixelimg', function () {
		event.preventDefault();
		event.stopPropagation();
		do_block_click();
	});
	...
	jQuery(document).off('mouseenter', '#block_pointer').on('mouseenter', '#block_pointer', function () {
		isOverBlockPointer = true;
	});

	jQuery(document).off('mouseleave', '#block_pointer').on('mouseleave', '#block_pointer', function () {
