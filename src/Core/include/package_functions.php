...
function display_package_options_table( $banner_id, $selected = '', $selection_ability = false ) {
	...
	$sql = "SELECT * FROM " . MDS_DB_PREFIX . "packages WHERE banner_id='$banner_id' ORDER BY price ASC ";
...
function banner_get_packages( $banner_id ) {
	...
	$sql = "SELECT * FROM " . MDS_DB_PREFIX . "packages WHERE banner_id=$banner_id";
	$result = mysqli_query( $GLOBALS['connection'], $sql ) or die ( mysqli_error( $GLOBALS['connection'] ) . $sql );
	if ( mysqli_num_rows( $result ) > 0 ) {
		return $result;
	}
...
function get_default_package( $banner_id ) {
	...
	$sql = "SELECT package_id FROM " . MDS_DB_PREFIX . "packages WHERE banner_id=$banner_id AND is_default='Y' ";
	$result = mysqli_query( $GLOBALS['connection'], $sql ) or die ( mysqli_error( $GLOBALS['connection'] ) . $sql );
	$row = mysqli_fetch_array( $result );
