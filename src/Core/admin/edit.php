...
<p>
	...
	if ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'changepass' ) {
		...
		if ( strcmp( $_REQUEST['password'], $_REQUEST['password2'] ) == 0 ) {
			...
			$sql = "UPDATE " . MDS_DB_PREFIX . "users set password='$newpass' where ID=" . intval( $user_id );
...
if ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'rank' ) {
	...
	$sql = "UPDATE " . MDS_DB_PREFIX . "users set Rank='" . mysqli_real_escape_string( $GLOBALS['connection'], $_REQUEST['rank'] ) . "' where ID=" . intval( $user_id );
	mysqli_query( $GLOBALS['connection'], $sql ) or die ( mysqli_error( $GLOBALS['connection'] ) );
//echo $sql;
