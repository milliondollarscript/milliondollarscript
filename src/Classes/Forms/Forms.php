...
class Forms {
	...
	public static function mds_form_submission(): void {
		...
		switch ( $mds_dest ) {
			...
			case 'select':
				...
				if ( isset( $_FILES['graphic'] ) && $_FILES['graphic']['tmp_name'] != '' ) {
					...
					// Log upload processing for debugging
					error_log( 'MDS Forms: Processing file upload, redirecting to order page with params: ' . print_r( $params, true ) );
				} else {
		...
		if ( empty( $page_url ) ) {
			...
			error_log( 'MDS Forms: Failed to generate page URL for destination: ' . $mds_dest );
			
			// Try to create a basic endpoint URL as final fallback
			$endpoint = \MillionDollarScript\Classes\Data\Options::get_option( 'endpoint', 'milliondollarscript' );
			...
			if ( empty( $page_url ) ) {
				$page_url = home_url();
				error_log( 'MDS Forms: All URL generation methods failed, falling back to homepage' );
			}
		}
		...
		try {
			...
			error_log( 'MDS Forms: Utility::redirect failed: ' . $e->getMessage() );
